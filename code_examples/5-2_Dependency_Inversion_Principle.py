...
# ------------------------------ 02 ------------------------------
# Создаем зависимости от интерфейса
...
class DataKeeperInterface(ABC):

    @abstractmethod
    def save_all(self, library: Library):
        pass

    @abstractmethod
    def get_all(self) -> Library:
        pass


class ConnectorToDBInterface(ABC):
    @abstractmethod
    def connect_to(db) -> DataKeeperInterface:
        pass


class ConnectorToDB(ConnectorToDBInterface):
    @staticmethod
    def connect_to(db: str = None) -> DataKeeperInterface:
        db_class = (
            globals()[db]
            if db else
            globals()[SETTINGS.get('db')]
        )
        return db_class()


class DatabaseMySQL(DataKeeperInterface):
...
class DatabasePostgreSQL(DataKeeperInterface):

    def _connect_to_postgresql(self):
        print(f'Подключились к БД PostgreSQL.')

    def save_all(self, library: Library):
        self._connect_to_postgresql()
        print(f'Сохранено в БД PostgreSQL.')

    def get_all(self) -> Library:
        self._connect_to_postgresql()
        print(f'Загружено из БД PostgreSQL.')
...
db = ConnectorToDB.connect_to()
buhgalter.libray = db.get_all()
db.save_all(buhgalter.libray)

print()
# Изменяем базу данных
SETTINGS['db'] = 'DatabasePostgreSQL'
